...
namespace OurHelper
{
    ...
    public class XMLHelper
    {
        ...
        public static int SetXmlAttributeValue(string xmlFileName, string xpath, string xmlAttributeName, String Value)
        {
            int content = 0;
            XmlDocument xmlDoc = new XmlDocument();
            XmlAttribute xmlAttribute = null;
            try
            {
                ...
                {
                    xmlAttribute = item.Attributes[xmlAttributeName];
...
#region ３.向XML文档中的子节点中新增或修改（如果存在则修改）一个子节点,比如name,author,date节点等：
//string xmlFileName = Application.StartupPath + @"\book.xml";
...
#region 4.向XML文档中的子节点中新增或修改（如果存在则修改）一个子节点属性,比如id,ISDN属性等：
