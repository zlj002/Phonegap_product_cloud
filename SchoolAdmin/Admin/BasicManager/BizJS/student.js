    ...
    $scope.bindList = function () {
        table = $('#tableList').dataTable({
            ...
            "aoColumns": [
                ...
                {
                    "sClass": "center", "mDataProp": "XH"
                },
                {
                    "sClass": "center", "mDataProp": "MobileNumber"
                }
            ],
            ...
            "sAjaxSource": "Handler/studentHandler.ashx?action=GetList&now=" + new Date().getDate(),
            //数据项的属性名，默认为aaData
            "sAjaxDataProp": "List",
            //服务器端，数据回调处理  
            ...
            "fnRowCallback": function (nRow, aData, iDataIndex) {
                ...
                var checkbox = $('<input title="选择" type="checkbox" id="check_' + aData.ID + '" name="check_' + aData.ID + '"  studentID="' + aData.ID + '"/>');
    ...
    $scope.btnDelete = function () {
        ...
        if (students.length > 0) {
            if (confirm("确定要删除吗？")) {
                ...
                var para = { "action": "Delete", "students": MY_JSON.stringify(students) };
                $.ajax({
                    type: 'POST',
                    url: "Handler/studentHandler.ashx/Delete",
                    data: para,
                    dataType: "json",
